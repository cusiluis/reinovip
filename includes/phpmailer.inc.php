...
class phpmailer {
   ...
   /**
    ...
    function create_header() {
        ...
        if(($this->Mailer != "mail") && (count($this->to) > 0)) $header[] = $this->addr_append("To", $this->to);
        ...
        if ($this->UseMSMailHeaders) $header[] = $this->AddMSMailHeaders();
    ...
    function attach_all() {
        ...
        for($i = 0; $i < count($this->attachment); $i++) {
            ...
            else $mime[] = sprintf("Content-ID: <%s>%s", $name, $this->LE);
            $mime[] = sprintf("Content-Disposition: %s; filename=\"%s\"%s", 
                              $disposition, $name, $this->LE.$this->LE);

            // Encode as string attachment
            if ($bString) {
                if(!$mime[] = $this->encode_string($string, $encoding)) return false;
                $mime[] = $this->LE.$this->LE;
            } else {
                if(!$mime[] = $this->encode_file($path, $encoding)) return false;
                $mime[] = $this->LE.$this->LE;
            }
        }
        $mime[] = sprintf("--%s--%s", $this->boundary[1], $this->LE);
    ...
    function encode_qp ($str) {
        $encoded = $this->fix_eol($str);
        if (substr($encoded, -2) != $this->LE) $encoded .= $this->LE;
